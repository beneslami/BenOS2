...
uint32_t amd_am79c973::HandleInterrupt(uint32_t esp){
    ...
    registerAddressPort.Write(0);
    uint32_t temp = registerDataPort.Read();
    if((temp & 0x8000) == 0x8000) printf("General Error\n");
    if((temp & 0x2000) == 0x2000) printf("Collision Error\n");
    if((temp & 0x1000) == 0x1000) printf("Missed Frame Error\n");
    if((temp & 0x0800) == 0x0800) printf("Memory Error\n");
    if((temp & 0x0400) == 0x0400) Receive();
    if((temp & 0x0200) == 0x0200) printf("Data Sent\n");
    registerAddressPort.Write(0);
    registerDataPort.Write(temp);
    if((temp & 0x0100) == 0x0100) printf("Initialization Done\n");
    return esp;
}
