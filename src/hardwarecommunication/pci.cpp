...
uint32_t PeripheralComponentInterconnectController::Read(uint16_t bus, uint16_t device, uint16_t function, uint32_t registeroffset){
    uint32_t id = 0x1 << 31 | ((bus & 0xFF) << 16) | ((device & 0x1F) << 16) | ((function & 0x07) << 8) | (registeroffset & 0xFC);
...
void PeripheralComponentInterconnectController::SelectDrivers(DriverManager *driverManager, BenOS::hardwarecommunication::InterruptManager *interrupts){
    ...
    switch(dev.class_id){
        case 0x03: // Graphics
            switch(dev.subclass_id){
                case 0x00:  //VGA
                    printf("VGA Device has been found\n");
                    break;
            }
            break;
    }
